...
def _read_export(path: Path) -> tuple[str, str]:
        ...
        try:
            with zipfile.ZipFile(path) as archive:
                ...
                for member in members:
                    ...
                    if member_path.is_absolute() or ".." in member_path.parts or member.filename.startswith(("/", "\\")):
    ...
    try:
        return data.decode("utf-8-sig"), source_name
...
def _parse_timestamp(date_text: str, time_text: str, day_first: bool) -> str:
    ...
    date_formats = ("%d/%m/%y", "%d/%m/%Y", "%m/%d/%y", "%m/%d/%Y") if day_first else ("%m/%d/%y", "%m/%d/%Y", "%d/%m/%y", "%d/%m/%Y")
    time_formats = ("%I:%M %p", "%I:%M:%S %p", "%H:%M", "%H:%M:%S")
    formats = tuple(f"{date_format} {time_format}" for date_format in date_formats for time_format in time_formats)
    for fmt in formats:
        try:
            return datetime.strptime(value, fmt).isoformat(timespec="seconds")
        except ValueError:
            pass
    raise ConfigError(f"unsupported WhatsApp timestamp: {date_text} {time_text}")


def parse_whatsapp_export(text: str) -> list[ImportedMessage]:
    matches = [BRACKET_RE.match(line) or MESSAGE_RE.match(line) for line in text.splitlines()]
    day_first = next((int(match.group("date").split("/")[0]) > 12 for match in matches if match and max(map(int, match.group("date").split("/")[:2])) > 12), False)
...
def import_whatsapp(config: AppConfig, path: Path, chat_id: str, name: str | None = None) -> dict[str, object]:
    init_db_path(config.archive.path)
    chat_id = chat_id.strip()
    with open_db(config.archive.path) as conn:
    ...
    text, source_name = _read_export(path)
    ...
    with open_db(config.archive.path) as conn:
        ...
        for position, message in enumerate(parsed):
            ...
            conn.execute(
                ...
                INSERT INTO messages(message_id, chat_id, sort_key, timestamp, sender_id, sender_name, is_sender,
                    message_type, text, normalized_text, raw_json, source_kind, source_ref, created_at, updated_at)
        ...
        conn.commit()
    return {"chat_id": chat_id, "chat_name": display_name, "source": source_name, "message_count": len(parsed)}
...
def scoped_search(config: AppConfig, chat_id: str, query: str, limit: int = 20) -> list[dict[str, object]]:
    ...
    with open_db(config.archive.path) as conn:
        sources = {str(row["message_id"]): {"kind": str(row["source_kind"]), "ref": str(row["source_ref"] or row["message_id"])}
                   for row in conn.execute("SELECT message_id, source_kind, source_ref FROM messages WHERE chat_id = ?", (chat_id,))}
...
def surrounding_thread(config: AppConfig, chat_id: str, message_id: str, radius: int = 3) -> list[dict[str, object]]:
    ...
    with open_db(config.archive.path) as conn:
        ...
        if anchor is None:
            return []
        columns = "m.message_id, m.sender_name, m.timestamp, m.text, m.source_kind, m.source_ref, m.sort_key"
        before = conn.execute(
